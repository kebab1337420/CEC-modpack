if not BotWeapons then
	...
	-- returns the npc factory id matching a factory id, the id itself if it already is one
	-- returns nil when no npc version exists, first person weapon units cannot be given to a bot
	function BotWeapons:to_npc_factory_id(factory_id)
		if not factory_id or not tweak_data.weapon.factory[factory_id] then
			return nil
		end
		if factory_id:match("_npc$") then
			return factory_id
		end
		if tweak_data.weapon.factory[factory_id .. "_npc"] then
			return factory_id .. "_npc"
		end
		BLT:Log(LogLevel.WARN, "[BWE] Weapon " .. tostring(factory_id) .. " has no npc version, bots cannot use it!")
		return nil
	end

	-- tells whether a part can be spawned on a weapon of that kind
	-- a part that has no unit for the kind of weapon it is assembled on, or a custom part whose unit
	-- is missing from the asset database, makes the engine spawn and link a unit that does not exist,
	-- which crashes the game (access violation in WeaponFactoryManager:_add_part) instead of raising
	-- a lua error, parts that carry no unit at all are stat only and always safe
	local function is_part_spawnable(part, is_npc)
		if not part then
			return false
		end
		...
		local unit_name = is_npc and part.third_unit or not is_npc and (part.unit or part.third_unit)
	...
	-- builds a blueprint that is safe to assemble on the given factory id, starting from its default
	-- blueprint and only keeping the parts that factory declares and can actually spawn
	function BotWeapons:sanitize_blueprint(factory_id, blueprint)
		local factory = tweak_data.weapon.factory
		local factory_data = factory_id and factory[factory_id]
		if not factory_data then
			return nil
		end
		local result = deep_clone(factory_data.default_blueprint or {})
		if not blueprint then
			return result
		end
		local is_npc = factory_id:match("_npc$") ~= nil
		local uses_parts = {}
		local has_part_list = false
		for _, part_id in pairs(factory_data.uses_parts or {}) do
			uses_parts[part_id] = true
			has_part_list = true
		end
		for _, part_id in pairs(blueprint) do
			if (not has_part_list or uses_parts[part_id]) and is_part_spawnable(factory.parts[part_id], is_npc) then
				managers.weapon_factory:change_part_blueprint_only(factory_id, part_id, result)
			else
				BLT:Log(LogLevel.WARN, "[BWE] Skipped weapon part " .. tostring(part_id) .. " for " .. tostring(factory_id) .. ", it cannot be assembled safely!")
			end
		end
		return result
	end

	-- returns the cosmetics only if they can be applied to the weapon without crashing
	function BotWeapons:sanitize_cosmetics(factory_id, cosmetics)
		local skin_id = cosmetics and cosmetics.id
		local skin = skin_id and skin_id ~= "nil" and tweak_data.blackmarket.weapon_skins[skin_id]
		if not skin then
			return nil
		end
		local weapon_id = managers.weapon_factory:get_weapon_id_by_factory_id(factory_id)
		weapon_id = weapon_id and (weapon_id:gsub("_npc$", ""))
		if skin.weapon_id and weapon_id and skin.weapon_id ~= weapon_id then
			BLT:Log(LogLevel.WARN, "[BWE] Weapon skin " .. tostring(skin_id) .. " does not belong to " .. tostring(weapon_id) .. ", skipped it!")
			return nil
		end
		local is_npc = factory_id:match("_npc$") ~= nil
		for _, part_id in pairs(skin.default_blueprint or {}) do
			if not is_part_spawnable(tweak_data.weapon.factory.parts[part_id], is_npc) then
				BLT:Log(LogLevel.WARN, "[BWE] Weapon skin " .. tostring(skin_id) .. " uses part " .. tostring(part_id) .. " which cannot be assembled, skipped the skin!")
				return nil
			end
		end
		return cosmetics
	...
	function BotWeapons:get_random_weapon(category)
		...
		-- the weapon table is a cache entry, clear the previous roll before filling it again
		...
		if math.random() < self.settings.weapon_cosmetics_chance then
			...
			if cosmetics then
				weapon.cosmetics = self:sanitize_cosmetics(weapon.factory_id, {
					id = cosmetics,
		...
		weapon.blueprint = self:sanitize_blueprint(weapon.factory_id, weapon.cosmetics and tweak_data.blackmarket.weapon_skins[weapon.cosmetics.id].default_blueprint)
		...
		for _, parts_data in pairs(managers.blackmarket:get_dropable_mods_by_weapon_id(weapon.weapon_id)) do
			if math.random() < self.settings.weapon_customized_chance then
				...
				if part_data and not forbidden[part_data[1]] then
					if is_part_spawnable(tweak_data.weapon.factory.parts[part_data[1]], true) then
	...
	function BotWeapons:get_loadout(char_name, original_loadout, refresh)
		...
		if not Utils:IsInGameState() or Network:is_server() then
			...
			-- bots always carry the npc version, and only parts that version can actually assemble
			loadout.primary = self:to_npc_factory_id(loadout.primary)
			if loadout.primary then
				if loadout.primary_blueprint then
					loadout.primary_blueprint = self:sanitize_blueprint(loadout.primary, loadout.primary_blueprint)
				end
				loadout.primary_cosmetics = self:sanitize_cosmetics(loadout.primary, loadout.primary_cosmetics)
			else
				loadout.primary_blueprint = nil
				loadout.primary_cosmetics = nil
			end
