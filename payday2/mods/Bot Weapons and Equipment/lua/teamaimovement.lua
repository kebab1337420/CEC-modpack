...
function TeamAIMovement:add_weapons()
	if Network:is_server() then
		...
		-- always assemble the npc version of the weapon, first person units cannot be linked to a bot
		local factory_id = BotWeapons:to_npc_factory_id(loadout.primary)
		if factory_id then
			local crafted = loadout.primary_slot and managers.blackmarket:get_crafted_category_slot(loadout.primary_category or "primaries", loadout.primary_slot)
			-- crafted blueprints come from the player inventory and may hold parts the npc weapon
			-- cannot spawn, which crashes the engine, so they are filtered before assembling
			local blueprint = BotWeapons:sanitize_blueprint(factory_id, crafted and crafted.blueprint or loadout.primary_blueprint)
			local cosmetics = BotWeapons:sanitize_cosmetics(factory_id, crafted and crafted.cosmetics or loadout.primary_cosmetics)
			self._unit:inventory():add_unit_by_factory_blueprint(factory_id, false, false, blueprint, cosmetics)
		else
