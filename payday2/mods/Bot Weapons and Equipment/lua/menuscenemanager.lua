function MenuSceneManager:set_henchmen_loadout(index, character, loadout)
	...
	if loadout.player_style == managers.blackmarket:get_default_player_style() then
		...
		if loadout.armor ~= "level_1" then
			-- the "armor_skin" extension is added to npc units by tweak/units.xml, which is disabled
			-- on Diesel 3.0 installs (.crate assets) where the xml tweaker cannot run, so this call
			-- may fail and must not break the crew display in the menu
			local ok, err = pcall(self.set_character_armor_skin, self, loadout.armor_skin, unit)
			if not ok then
				BLT:Log(LogLevel.WARN, "[BWE] Armor skin not applied to bot (armor_skin extension missing): " .. tostring(err))
			end
		end
	end
	...
	if weapon_id and tweak_data.weapon[weapon_id] and tweak_data.weapon[weapon_id].categories[1] == "akimbo" then
