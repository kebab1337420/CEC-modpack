...
-- TEMP DIAGNOSTIC PROBE: trace the host side of a join / leave / rejoin
function BigLobbyGlobals:join_log(msg)
	local f = io.open((self.SavePath or "") .. "biglobby_join.log", "a")
	if f then
		local stamp = ""
		pcall(function() stamp = os.date("%H:%M:%S") .. " " end)
		f:write(stamp .. tostring(msg) .. "\n")
		f:close()
	end
end

local function peer_summary(session)
	local out = {}
	for id, peer in pairs(session and session._peers or {}) do
		table.insert(out, ("%s(%s,synced=%s,loading=%s,lobby=%s)"):format(
			tostring(id), tostring(peer:name()), tostring(peer:synched()),
			tostring(peer:loading()), tostring(peer:in_lobby())))
	end

	return table.concat(out, " ")
end

local probe__on_join_request_received = HostStateInLobby.on_join_request_received

function HostStateInLobby:on_join_request_received(data, peer_name, peer_account_type_str, peer_account_id, is_invite, ...)
	BigLobbyGlobals:join_log(("JOIN REQ name=%s account=%s invite=%s slots=%s peers=[%s]"):format(
		tostring(peer_name), tostring(peer_account_id), tostring(is_invite),
		tostring(BigLobbyGlobals:num_player_slots()), peer_summary(data.session)))

	return probe__on_join_request_received(self, data, peer_name, peer_account_type_str, peer_account_id, is_invite, ...)
end

local probe__on_join_auth_received = HostStateInLobby.on_join_auth_received

function HostStateInLobby:on_join_auth_received(data, auth_ticket, sender)
	BigLobbyGlobals:join_log("AUTH RECEIVED peers=[" .. peer_summary(data.session) .. "]")

	return probe__on_join_auth_received(self, data, auth_ticket, sender)
end

local probe__send_request_denied = HostStateInLobby._send_request_denied

function HostStateInLobby:_send_request_denied(sender, reply, my_user_id, ...)
	BigLobbyGlobals:join_log("DENIED reply=" .. tostring(reply) .. " stack=" .. tostring(Application:stack()))

	return probe__send_request_denied(self, sender, reply, my_user_id, ...)
end
