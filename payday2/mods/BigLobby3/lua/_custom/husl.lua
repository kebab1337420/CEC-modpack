-- HUSLP implementation from: https://github.com/husl-colors/husl-lua/blob/master/husl.lua
-- I've modified it so all variables are tied to the `husl` table. PD2 also seems
-- to return degrees and use degrees as input instead of radians for math functions.
-- I've made adjustments to these methods commenting out the original to correct this.
if not _G.HUSL then
	-- Version: 1.0.0
	-- public api
	...
	husl._refY = 1.00000
	husl._refZ = 1.08883
	husl._refU = (4 * husl._refX) / (husl._refX + (15 * husl._refY) + (3 * husl._refZ))
	husl._refV = (9 * husl._refY) / (husl._refX + (15 * husl._refY) + (3 * husl._refZ))

	husl._kappa = 24389 / 27
	husl._epsilon = 216 / 24389
	...
	function husl.max_chroma(L, H)
	    ...
	    local sub2 = (sub1 > husl._epsilon) and sub1 or (L / husl._kappa)

	    local result = math.huge

	    for _, row in ipairs(husl._m) do
	...
	function husl.hrad_extremum(L)
	    local lhs = (math.pow(L, 3) + 48 * math.pow(L, 2) + 768 * L + 4096) / 1560896
	    local rhs = husl._epsilon
	    local sub = (lhs > rhs) and lhs or L / husl._kappa
	    ...
	    for _, row in ipairs(husl._m) do
	        ...
	        for limit=0,1 do
				...
				local hrad = math.rad(math.atan2(top, bottom))
	...
	function husl.max_chroma_pastel(L)
	    local H = math.deg(husl.hrad_extremum(L))

	    return husl.max_chroma(L, H)
	end

	function husl.dot_product(a, b)
	    local sum = 0
	...
	function husl.f(t)
	    if t > husl._epsilon then
	        return 116 * math.pow((t / husl._refY), 1 / 3) - 16
	    else
	        return t / husl._refY * husl._kappa
	    end
	end

	function husl.f_inv(t)
	    if t > 8 then
	        return husl._refY * math.pow((t + 16) / 116, 3)
	    else
	        return husl._refY * t / husl._kappa
	    end
	end
	...
	function husl.round(number, digits)
	...
	function husl.xyz_to_rgb(x, y, z)
	    ...
	    for i, row in ipairs(husl._m) do
	        rgb[i] = husl.from_linear(husl.dot_product(row, {x, y, z}))
	    end
	...
	function husl.rgb_to_xyz(r, g, b)
	    ...
	    for i, row in ipairs(m_inv) do
	        xyz[i] = husl.dot_product(row, rgb)
	    end
	...
	function husl.xyz_to_luv(X, Y, Z)
	    ...
	    local U = 13 * L * (varU - husl._refU)
	    local V = 13 * L * (varV - husl._refV)
	...
	function husl.luv_to_xyz(L, U, V)
	    ...
	    local varU = U / (13 * L) + husl._refU
	    local varV = V / (13 * L) + husl._refV
	    local Y = varY * husl._refY
