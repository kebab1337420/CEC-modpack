...
if not _G.VehicleCamera or CMCG.vehicle_camera_patched then
	return
end

CMCG.vehicle_camera_patched = true

-- Stock VehicleCamera:init() allocates, for every vehicle unit the level
-- spawns and at spawn time:
--
--   World:create_camera()
--   managers.viewport:new_vp(0, 0, 1, 1, "VehicleCamera", PRIO_WORLDCAMERA)
--   viewport:director(), director:shaker(), director:make_camera(...)
--
-- A vanilla heist has one or two vehicles, so nobody ever noticed. This level
-- spawns 28, so the viewport manager ends up walking 29 viewports every frame,
-- each with its own camera and director, none of them ever active. That loop is
-- where the access violation lands.
--
-- Nothing in the base game touches a vehicle camera until the player is in the
-- driving state (playerdriving.lua: show_next / set_rear_cam_active /
-- rear_cam_active), so the whole thing is built on first use instead of at
-- spawn. A vehicle nobody drives costs one table.
...
function VehicleCamera:init(unit)
	...
	if deferred % 10 == 0 then
		CMCG:write("vehiclecamera: %d viewport(s) deferred", deferred)
	end
end

-- Everything init() used to do, done the first time the camera is really used.
function VehicleCamera:_ensure_viewport()
	...
	self._camera:set_fov(self._fov or self._default_fov)
	...
	CMCG:write("vehiclecamera: viewport built on demand for %s", tostring(alive(self._unit) and self._unit:name() or "?"))

	return true
end

function VehicleCamera:_setup_sound_listener()
	if not self:_ensure_viewport() then
		return
	end
...
function VehicleCamera:update_camera()
	if not self._active or not self._camera_controller then
		return
	end
...
CMCG:write("vehiclecamera: lazy viewport patch active")
