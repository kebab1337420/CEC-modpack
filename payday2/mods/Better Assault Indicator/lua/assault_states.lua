...
if cache.level_id == "pbr2" and BAI:IsHost() then -- Other detection method is in "GroupAIStateBase.lua" file
    Hooks:PostHook(GroupAIStateBesiege, "_upd_recon_tasks", "BAI_GroupAIStateBesiege__upd_recon_tasks", function(self)
        if self._task_data.recon.tasks and self._task_data.recon.tasks[1] then
            BAI:UpdateAssaultState("control", true, true)
            Hooks:RemovePostHook("BAI_GroupAIStateBesiege__upd_recon_tasks")
...
Hooks:PostHook(HUDManager, "sync_start_anticipation_music", "BAI_HUDManager_sync_start_anticipation_music", function(...)
...
function BAI:SetAssaultStatesHook(hook)
    if hook then
        Hooks:PostHook(GroupAIStateBesiege, "_upd_assault_task", "BAI_GroupAIStateBesiege__upd_assault_task", phase)
    else
        Hooks:RemovePostHook("BAI_GroupAIStateBesiege__upd_assault_task")
    end
end
