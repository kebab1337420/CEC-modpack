...
function BAI:ApplyHUDCompatibility(number, detection)
    ...
    elseif number == 4 then
        ...
        Hooks:RemovePostHook("_upd_recon_tasks_PostHook")
    elseif number == 5 then
        params.hud_filename = "holoui"
        DelayedCalls:Add("bai_holoui_compatibility", 2, function()
            Hooks:RemovePostHook("HoloUI_start_assault")
            Hooks:RemovePostHook("HoloUI_end_assault")
        end)
    ...
    elseif number == 10 then
        ...
        params.vanilla = true
        Hooks:RemovePostHook("khud_detect_assaultphase")
        Hooks:RemovePostHook("khud_detect_regroupphase")
...
function BAI:ApplyModCompatibility(number, detection)
    if number == 1 then
        if NetworkMatchMakingSTEAM and NetworkMatchMakingSTEAM._BUILD_SEARCH_INTEREST_KEY == "HYPERHEISTSHIN" then
...
function BAI:RemoveASCalls()
    DelayedCalls:Remove("BAI_AssaultStateChange_Sustain")
    DelayedCalls:Remove("BAI_AssaultStateChange_Fade")
end