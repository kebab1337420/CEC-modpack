...
---@class BAIMenuItemColor : CoreMenuItem.Item
BAIMenuItemColor = class(CoreMenuItem.Item)
function BAIMenuItemColor:init(data_node, parameters)
...
function BAIMenuItemColor:setup_gui(node, row_item)
...
function BAIMenuItemColor:_layout(node, row_item)
...
function BAIMenuItemColor:reload(row_item, node)
...
function BAIMenuItemColor:highlight_row_item(node, row_item, mouse_over)
    row_item.gui_text:set_color(row_item.color)
    return true
end

function BAIMenuItemColor:fade_row_item(node, row_item, mouse_over)
    row_item.gui_text:set_color(row_item.color)
    return true
end

---@param value { r: integer, g: integer, b: integer }
function BAIMenuItemColor:set_value(value)
    for _, option in ipairs(self._options) do
        local o_params = option:parameters()
...
function BAIMenuItemColor:value()
    local color = {}
    for _, option in ipairs(self._options) do
        local o_params = option:parameters()
...
BAIMenuSetColorInitiator = class(MenuCrimeNetSpecialInitiator)
function BAIMenuSetColorInitiator:modify_node(original_node, data)
    return self:setup_node(original_node, data)
end

function BAIMenuSetColorInitiator:setup_node(node, data)
    node:clean_items()
    ...
    local params = {
        callback = "bai_reset_color_params",
        name = "reset",
        text_id = "bai_color_reset",
        align = "right"
    }
    node:add_item(node:create_item({}, params))
...
function BAIMenuSetColorInitiator:refresh_node(node, data)
    return node
end

function BAIMenuSetColorInitiator:create_slider(node, params)
    params.callback = "bai_update_color"
...
function MenuCallbackHandler:bai_update_color()
...
function MenuCallbackHandler:bai_reset_color_params()
    local menu = managers.menu:active_menu()
    if not menu then
        return false
    end
...
BAIMenuNodeCustomizeGadgetGui = BAIMenuNodeCustomizeGadgetGui or class(MenuNodeGui)
BAIMenuNodeCustomizeGadgetGui._rec_round_object = MenuNodeCustomizeGadgetGui._rec_round_object
local padding = 10

function BAIMenuNodeCustomizeGadgetGui:init(node, layer, parameters)
    ...
    BAIMenuNodeCustomizeGadgetGui.super.init(self, node, layer, parameters)
    self:setup(node)
end

function BAIMenuNodeCustomizeGadgetGui:setup(node, default_color)
    local r = node:item("red")
    local g = node:item("green")
    local b = node:item("blue")
...
function BAIMenuNodeCustomizeGadgetGui:make_fine_text(text)
...
function BAIMenuNodeCustomizeGadgetGui:_setup_item_panel(safe_rect, res)
    BAIMenuNodeCustomizeGadgetGui.super._setup_item_panel(self, safe_rect, res)
...
function BAIMenuNodeCustomizeGadgetGui:update_node_colors()
...
function BAIMenuNodeCustomizeGadgetGui:_setup_item_panel_parent(safe_rect, shape)
    shape = shape or {}
    ...
    BAIMenuNodeCustomizeGadgetGui.super._setup_item_panel_parent(self, safe_rect, shape)
end

function BAIMenuNodeCustomizeGadgetGui:reload_item(item)
    BAIMenuNodeCustomizeGadgetGui.super.reload_item(self, item)
...
function BAIMenuNodeCustomizeGadgetGui:_align_marker(row_item)
    BAIMenuNodeCustomizeGadgetGui.super._align_marker(self, row_item)
