...
function HUDAssaultCorner:ApplyCompatibility()
    ...
    BAI:AddEvent(BAI.EventList.Update, function()
        ...
        for _, icon in ipairs(self._icons) do
            if icon.id == "captain" then
                icon.position = BAI:IsHostagePanelVisible("captain") and 3 or 1
                break
            end
        end
    end)
...
function HUDAssaultCorner:InitCaptainPanel()
    ...
    local panel_h = 38 * self._scale
    local panel_w = 44 * self._scale
    local position = BAI:IsHostagePanelVisible("captain") and 3 or 1
    local icon_color = nil
    local text_color = Color.white
    if VoidUI_HMV then
        icon_color = VoidUI.options.generic_colors and VoidUI:GetColor("text_color") or VoidUI:GetColor("cuffed_icon")
    ...
    self._captain_panel = self._custom_captain_panel:panel({
        name = "captain_panel",
        w = panel_w,
        h = panel_h,
        alpha = 0
    ...
    table.insert(icons, {panel = captain_panel, position = position, id = "captain"})
    ...
    BAI:AddEvent(BAI.EventList.Captain, function(active)
        self.assault_type = active and "captain" or "assault"
    end, nil, 100)
end
...
function HUDAssaultCorner:_offset_hostage(is_offseted, hostage_panel, big_logo)
    ...
    local captain_panel = self._custom_captain_panel
    ...
    while TOTAL_T > t do
        ...
        if is_offseted then
            ...
            local right = math.lerp(panel_right, self._custom_hud_panel:w(),lerp)
            icons_panel:set_right(right)
            captain_panel:set_right(right)
            local y = math.lerp(panel_y, 0, lerp)
            icons_panel:set_y(y)
            captain_panel:set_y(y)
    ...
    if is_offseted then
        ...
        TOTAL_T = 0.3
        t = 0
        local right = big_logo and self._custom_hud_panel:w() - 75 * self._scale or self._custom_hud_panel:w() - 7 * self._scale
        icons_panel:set_right(right)
        captain_panel:set_right(right)
        local y = big_logo and 47 * self._scale or 32 * self._scale
        icons_panel:set_y(y)
        captain_panel:set_y(y)
        for _, panels in ipairs(self._icons) do
            panels.panel:set_right(icons_panel:w() - (panels.position - 1) * panels.panel:w() - 4 * (panels.row and panels.row or 0))
...
function HUDAssaultCorner:_start_assault(text_list)
    ...
    if assaultbox_panel:child("text_panel") then
        ...
        assaultbox_panel:child("text_panel"):set_w(self._badge and assaultbox_panel:w() or assaultbox_panel:w() - 26 * self._scale)
    else
        assaultbox_panel:panel({name = "text_panel", w = self._badge and assaultbox_panel:w() or assaultbox_panel:w() - 30 * self._scale})
    end
    local text_panel = assaultbox_panel:child("text_panel")

    assault_panel:set_visible(true)
    icon_assaultbox:set_visible(self._badge)
    if assaultbox_skulls then
        assaultbox_skulls:set_visible(self._badge)
    end
