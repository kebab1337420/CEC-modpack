...
function HUDAssaultCorner:_set_hostage_offseted(is_offseted)
    if self.assault_panel_position == 1 then -- Top Left
        ...
        if HUDListManager and managers.hudlist then -- Don't trigger assault start twice
            local list = managers.hudlist:list("left_list")
            if list and list._panel then
                local l_panel = list._panel
                l_panel:stop()
                l_panel:animate(callback(self, self, "_offset_hostages"), is_offseted, panel_h)
            end
        end
...
function HUDAssaultCorner:_set_hostages_offseted(is_offseted)
    if self.assault_panel_position == 1 then -- Top Left
        ...
        if HUDListManager and managers.hudlist then
            local list = managers.hudlist:list("left_list")
            if list and list._panel then
                local l_panel = list._panel
                l_panel:stop()
...
function HUDAssaultCorner:_show_hostages(no_animation)
    if not self._point_of_no_return and self.AnimateHostagesPanel and not no_animation then
        if self.compatibility_flags.AnimateNew then
            BAI:AnimateSafe("HostagePanelVisibility", 1, callback(self, self, "ShowHostagePanel"), self, self._hud_panel:child("hostages_panel"))
        else
...
function HUDAssaultCorner:_hide_hostages(no_animation)
    if self.AnimateHostagesPanel and not no_animation then
        if self.compatibility_flags.AnimateNew then
            BAI:AnimateSafe("HostagePanelVisibility", 0, callback(self, self, "HideHostagePanel"), self, self._hud_panel:child("hostages_panel"))
        else
...
function HUDAssaultCorner:SetHook(hook, delay)
    if hook then
        ...
        if BAI:GetAAIOption("show_time_left") then
            if self.compatibility_flags.AnimateNew then
                BAI:AnimateSafe("TimePanelVisibility", 1, callback(self, self, "ShowTimePanel"), self, self._time_left_panel, self._time_bg_box)
            else
                local function f()
                    BAI:Animate(self._time_left_panel, 1, self.AAIFunction, unpack(self.AAIFunctionArgs1))
                end
                DelayedCalls:Add("DelayTimeLeftPanel", delay or 1, f)
            end
        end
        if BAI:GetAAIOption("show_spawns_left") then
            if self.compatibility_flags.AnimateNew then
                BAI:AnimateSafe("SpawnsPanelVisibility", 1, callback(self, self, "ShowSpawnsPanel"), self, self._spawns_left_panel, self._spawns_bg_box)
            else
                local function f()
                    BAI:Animate(self._spawns_left_panel, 1, self.AAIFunction, unpack(self.AAIFunctionArgs2))
                end
                DelayedCalls:Add("DelaySpawnsLeftPanel", delay or 1, f)
            end
        end
    else
        ...
        local_t = 0
        if self.compatibility_flags.AnimateNew then
            BAI:AnimateSafe("TimePanelVisibility", 0, callback(self, self, "HideTimePanel"), self, self._time_left_panel)
            BAI:AnimateSafe("SpawnsPanelVisibility", 0, callback(self, self, "HideSpawnsPanel"), self, self._spawns_left_panel)
        else
            DelayedCalls:Remove("DelayTimeLeftPanel")
            DelayedCalls:Remove("DelaySpawnsLeftPanel")
...
function HUDAssaultCorner:SetBreakHook(hook, delay)
    if hook then
        ...
        if self.compatibility_flags.AnimateNew then
            BAI:AnimateSafe("BreakTimePanelVisibility", 1, callback(self, self, "ShowBreakTimePanel"), self, self._break_time_panel, self._break_time_bg_box)
        else
            local function f()
                BAI:Animate(self._break_time_panel, 1, self.AAIFunction, unpack(self.AAIFunctionArgs3))
            end
            DelayedCalls:Add("DelayBreakTimePanel", delay or 1, f)
        end
    else
        managers.hud:remove_updator("BAI_UpdateBreakTimeInfo")
        local_t_break = 0
        if self.compatibility_flags.AnimateNew then
            BAI:AnimateSafe("BreakTimePanelVisibility", 0, callback(self, self, "HideBreakTimePanel"), self, self._break_time_panel)
        else
            DelayedCalls:Remove("DelayBreakTimePanel")
            local function f()
                BAI:Animate(self._break_time_panel, 0, "FadeOut")
            end
            DelayedCalls:Add("HideBreakPanel", 1, f)
        end
    end
end
...
function HUDAssaultCorner:SetCaptainHook(active)
    ...
    if self.compatibility_flags.AnimateNew then
        BAI:AnimateSafe("CaptainPanelVisibility", active and 1 or 0, callback(self, self, active and "ShowCaptainPanel" or "HideCaptainPanel"), self, self._captain_panel)
    else
