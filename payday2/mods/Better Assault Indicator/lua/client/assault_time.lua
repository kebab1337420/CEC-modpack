...
function BAI:SetTimer()
    ...
    if not self.CompatibleHost then
        ...
        DelayedCalls:Add("BAI_AssaultStateChange_Sustain", tweak.build_duration, function()
            BAI:UpdateAssaultState("sustain")
        end)
        DelayedCalls:Add("BAI_AssaultStateChange_Fade", tweak.build_duration + sustain, function()
